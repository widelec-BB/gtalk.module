...
VOID HandleFeatures(struct ObjData *d, int features, iks *node)
{
	if(features)
	{
		...
		{
			STRPTR atoken = (STRPTR)xget(findobj(USD_PREFS_OAUTH_GROUP, d->PrefsPanel), OAGA_AuthToken);

			if((features & IKS_STREAM_OAUTH_2) && atoken)
				iks_start_sasl(d->StreamParser, IKS_SASL_OAUTH_2, d->Id->user, atoken);
			else
				AddErrorEvent(&d->EventsList, ERRNO_LOGIN_FAILED, GetString(MSG_ERROR_UNSUPPORTED_AUTH_METHOD));
		}
	}
...
VOID HandleIq(struct ObjData *d, ikspak *pac)
{
	...
	{
		iks *bind = iks_child(pac->x);

		if(bind && iks_strcmp(iks_name(bind), "bind") == 0)
		{
			iksid *n;

			if((n = iks_id_new(d->IksStack, iks_find_cdata(bind, "jid"))))
			{
				d->Id = n;
				AddEventStatusChange(&d->EventsList, d->StatusOnConnect, d->DescriptionOnConnect);
			}
			return;
		}
...
VOID HandleMessage(struct ObjData *d, ikspak *pac)
{
	iks *child;

	if(iks_find_cdata(pac->x, "body"))
	{
		if(pac->subtype != IKS_TYPE_GROUPCHAT)
			AddEventNewMsg(&d->EventsList, pac);
	}

	if((child = iks_child(pac->x)))
	{
		if(!iks_strncasecmp(iks_name(child), "cha:", 4))
		{
			if(!iks_strncasecmp(iks_name(child) + 4, "composing", 9))
				AddEventTypingNotify(&d->EventsList, pac->from->partial);
		}
	}
}
