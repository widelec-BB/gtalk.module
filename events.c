...
VOID AddEventStatusChange(struct MinList *list, enum ikshowtype s, STRPTR desc)
{
	...
	if((event = AddEvent(list, KE_TYPE_STATUS_CHANGE)))
	{
		...
		event->ke_StatusChange.ke_NewStatus = StatusTranslateToKwaKwa(s);
	}
}
...
VOID AddEventTypingNotify(struct MinList *list, STRPTR jid)
{
	...
	{
		if((event->ke_ListChange.ke_ContactID = StrNew(jid)))
		{
			event->ke_TypingNotify.ke_TxtLen = 667;
			return;
		}
		Remove((struct Node*)event);
		FreeKwaEvent(event);
...
VOID AddEventNotifyBeacon(struct MinList *list, STRPTR name, STRPTR msg, BOOL wait_res, ULONG methodid, APTR userdata)
{
	...
	if((event = AddEvent(list, KE_TYPE_NOTIFY_BEACON)))
	{
		if((event->ke_NotifyBeacon.ke_NotificationName = StrNew(name)))
		{
			if((event->ke_NotifyBeacon.ke_Message = StrNew(msg)))
			{
				event->ke_NotifyBeacon.ke_WaitForResult = wait_res;
				event->ke_NotifyBeacon.ke_MethodID = methodid;
				event->ke_NotifyBeacon.ke_UserData = userdata;

				return;
			}
			StrFree(event->ke_NotifyBeacon.ke_NotificationName);
		}
		Remove((struct Node*)event);
		FreeKwaEvent(event);
	}
